...
export class API_gestor {
    ...
    private async initDH() {
        try {
            ...
            if (response.ok && responseData.success && responseData.publicKey) {
                ...
                this.sharedKey = sharedSecret; //to do debug (non uguale a quella nel server)
    ...
    private async AES_encrypt(key: CryptoKey, plaintext: string): Promise<{ iv: string; encryptedData: string; authTag: string }> {
