            ...
            <div v-if="categorizedInventory.avatar.length > 0" class="inventory-category">
                ...
                <div class="inventory-grid">
                    <div v-for="inventoryItem in categorizedInventory.avatar" :key="inventoryItem.item.id"
                         ...
                         <img :src="inventoryItem.item.imageUrl || '/path/to/default-avatar-image.png'" :alt="inventoryItem.item.name" class="item-image" />
                         <div class="item-info">
                        ...
                        <button class="baseButton apply-button" @click="emitApplyItem(inventoryItem.item)">Apply</button>
                    </div>
                </div>
            </div>
            ...
            <div v-if="categorizedInventory.cosmetic.length > 0" class="inventory-category">
                ...
                <div class="inventory-grid">
                    <div v-for="inventoryItem in categorizedInventory.cosmetic" :key="inventoryItem.item.id"
                         :class="['inventory-item-card', `rarity-${inventoryItem.item.rarity}`]">
                         <img :src="inventoryItem.item.imageUrl || '/path/to/default-cosmetic-image.png'" :alt="inventoryItem.item.name" class="item-image" />
                         <div class="item-info">
                            <h4 :class="`rarity-text-${inventoryItem.item.rarity}`">{{ inventoryItem.item.name }}</h4>
                            <p>{{ inventoryItem.item.description }}</p>
                             <p v-if="inventoryItem.quantity > 1">Quantity: {{ inventoryItem.quantity }}</p>
            ...
            <div v-if="categorizedInventory.utility.length > 0" class="inventory-category">
                ...
                <div class="inventory-grid">
                    <div v-for="inventoryItem in categorizedInventory.utility" :key="inventoryItem.item.id"
                         :class="['inventory-item-card', `rarity-${inventoryItem.item.rarity}`]">
                         <img :src="inventoryItem.item.imageUrl || '/path/to/default-utility-image.png'" :alt="inventoryItem.item.name" class="item-image" />
                         <div class="item-info">
                            <h4 :class="`rarity-text-${inventoryItem.item.rarity}`">{{ inventoryItem.item.name }}</h4>
                            <p>{{ inventoryItem.item.description }}</p>
                             <p v-if="inventoryItem.quantity > 0">Quantity: {{ inventoryItem.quantity }}</p>
                         </div>
                         <button v-if="inventoryItem.quantity > 0" class="baseButton use-button" @click="emitUseItem(inventoryItem.item)">Use ({{ inventoryItem.quantity }})</button>
                         <p v-else>Used Up</p> </div>
                </div>
            </div>
             ...
             <div v-if="categorizedInventory.gift.length > 0" class="inventory-category">
                ...
                <div class="inventory-grid">
                    <div v-for="inventoryItem in categorizedInventory.gift" :key="inventoryItem.item.id"
                         :class="['inventory-item-card', `rarity-${inventoryItem.item.rarity}`]">
                         <img :src="inventoryItem.item.imageUrl || '/path/to/default-gift-image.png'" :alt="inventoryItem.item.name" class="item-image" />
                         <div class="item-info">
                            <h4 :class="`rarity-text-${inventoryItem.item.rarity}`">{{ inventoryItem.item.name }}</h4>
                            <p>{{ inventoryItem.item.description }}</p>
                             <p v-if="inventoryItem.quantity > 0">Quantity: {{ inventoryItem.quantity }}</p>
                         </div>
                         <button v-if="inventoryItem.quantity > 0" class="baseButton use-button" @click="emitUseItem(inventoryItem.item)">Gift ({{ inventoryItem.quantity }})</button>
                        <p v-else>Used Up</p>
                    </div>
                </div>
            </div>
             ...
             <div v-if="categorizedInventory.reminder.length > 0" class="inventory-category">
                ...
                <div class="inventory-grid">
                    <div v-for="inventoryItem in categorizedInventory.reminder" :key="inventoryItem.item.id"
                         :class="['inventory-item-card', `rarity-${inventoryItem.item.rarity}`]">
                         <img :src="inventoryItem.item.imageUrl || '/path/to/default-reminder-image.png'" :alt="inventoryItem.item.name" class="item-image" />
                         <div class="item-info">
                            <h4 :class="`rarity-text-${inventoryItem.item.rarity}`">{{ inventoryItem.item.name }}</h4>
                            <p>{{ inventoryItem.item.description }}</p>
                             <p v-if="inventoryItem.quantity > 0">Quantity: {{ inventoryItem.quantity }}</p>
...
const emit = defineEmits(['apply-item', 'use-item', "show-notification"]);

// Props - Now only receiving licenseKey and allItems
const props = defineProps<{
    userLicenseKey: string; // Prop for the user's license key
}>();
...
.loading-message .spinner {
     ...
     margin: 15px auto; /* Center spinner */
}
...
.inventory-category h3 {
    margin-bottom: 15px;
    color: var(--color); /* Or a different color for category titles */
...
.inventory-grid {
    ...
    padding-bottom: 20px; /* Space at the bottom of the grid */
}
...
.item-image {
    width: 70px; /* Slightly smaller image than shop */
    height: 70px;
    object-fit: cover;
    border-radius: 4px;
    margin-bottom: 10px;
...
.rarity-quite-common { border-color: #a4e4a4; }
...
.custom-scrollbar::-webkit-scrollbar {
    ...
    height: 8px; /* Horizontal scrollbar height (not used here, but good to have) */
}
...
.custom-scrollbar::-webkit-scrollbar-thumb:hover {
    background: var(--accent-color-dark); /* Darker thumb on hover */
}
